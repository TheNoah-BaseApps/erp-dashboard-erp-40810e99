...
export default function ActualSalesEditPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = id === 'new' ? '/api/actual-sales' : `/api/actual-sales/${id}`;
      ...
      if (response.ok && data.success) {
        toast.success(id === 'new' ? 'Actual sales created' : 'Actual sales updated');
        router.push('/actual-sales');
      } else {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="outline" onClick={() => router.push('/actual-sales')}>
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back
            </Button>
            <h1 className="text-3xl font-bold">
              {id === 'new' ? 'New Actual Sales' : 'Edit Actual Sales'}
            </h1>
          </div>
          {id !== 'new' && (
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" onClick={() => router.push('/actual-sales')}>
                  Cancel
                </Button>
